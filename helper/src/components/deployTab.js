...
export default function DeployTab({ defaults, updateFn, tabValues, invalidArray, invalidTabs, urlParams }) {
  ...
  const params = {
    ...
    ...(cluster.enable_aad && cluster.AksDisableLocalAccounts !== defaults.cluster.AksDisableLocalAccount && { AksDisableLocalAccounts: cluster.AksDisableLocalAccounts }),
    ...
    <Stack tokens={{ childrenGap: 15 }} styles={adv_stackstyle}>
      ...
      <Stack horizontal styles={{ root: { width: "100%" } }} tokens={{ childrenGap: 50 }}>
        <Stack tokens={{ childrenGap: 10 }} styles={{ root: { width: "400px" } }}>
          ...
          <TextField inputProps={{ "data-testid": "deploy-resourceGroup"}} id="azResourceGroup" label="Resource Group" onChange={(ev, val) => updateFn('rg', val)} required errorMessage={getError(invalidArray, 'rg')} value={deploy.rg} />
