...
class SnapshotHelper {
  constructor(testInfo, snapshotPathResolver, anonymousSnapshotExtension, configOptions, nameOrOptions, optOptions) {
    ...
    this.comparator = (0, _comparators.getComparator)(this.mimeType);
...
function toMatchSnapshot(received, nameOrOptions = {}, optOptions = {}) {
  ...
  if (testInfo.config._ignoreSnapshots) return {
    pass: !this.isNot,
    message: () => ''
  };
  const helper = new SnapshotHelper(testInfo, testInfo.snapshotPath.bind(testInfo), determineFileExtension(received), ((_testInfo$project$_ex = testInfo.project._expect) === null || _testInfo$project$_ex === void 0 ? void 0 : _testInfo$project$_ex.toMatchSnapshot) || {}, nameOrOptions, optOptions);
  if (this.isNot) {
...
async function toHaveScreenshot(pageOrLocator, nameOrOptions = {}, optOptions = {}) {
  var _testInfo$project$_ex2, _config$animations, _config$scale, _config$caret;
  const testInfo = (0, _globals.currentTestInfo)();
  ...
  const config = (_testInfo$project$_ex2 = testInfo.project._expect) === null || _testInfo$project$_ex2 === void 0 ? void 0 : _testInfo$project$_ex2.toHaveScreenshot;
  ...
  const customStackTrace = (0, _util.captureStackTrace)(`expect.toHaveScreenshot`);
  ...
  if (this.isNot) {
    ...
    const isDifferent = !(await page._expectScreenshot(customStackTrace, {
      ...
      screenshotOptions,
      timeout: (0, _util.currentExpectTimeout)(helper.allOptions)
    })).errorMessage;
    return isDifferent ? helper.handleDifferentNegated() : helper.handleMatchingNegated();
  }
  ...
  if (helper.updateSnapshots === 'none' && !hasSnapshot) return {
    pass: false,
    message: () => `${helper.snapshotPath} is missing in snapshots.`
  };
  if (!hasSnapshot) {
    // Regenerate a new screenshot by waiting until two screenshots are the same.
    const timeout = (0, _util.currentExpectTimeout)(helper.allOptions);
    const {
      actual,
      previous,
      diff,
