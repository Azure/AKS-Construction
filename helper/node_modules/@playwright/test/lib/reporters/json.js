...
class JSONReporter {
  ...
  _mergeSuites(suites) {
    const fileSuites = new _multimap.MultiMap();
    for (const projectSuite of suites) {
      const projectId = projectSuite.project()._id;
  ...
  _serializeError(error) {
    return (0, _base.formatError)(this.config, error, true);
  }
...
function outputReport(report, config, outputFile) {
